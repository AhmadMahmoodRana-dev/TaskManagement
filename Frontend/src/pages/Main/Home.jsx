...
const Home = () => {
  const [tasks, setTasks] = useState([
    { id: 1, title: 'Complete project proposal', category: 'Work', priority: 'High', completed: false, date: '2023-05-15', description: 'Finalize and submit proposal to client' },
    { id: 2, title: 'Team meeting with design', category: 'Work', priority: 'Medium', completed: false, date: '2023-05-16', description: 'Discuss UI improvements' },
    { id: 3, title: 'Buy groceries', category: 'Personal', priority: 'Medium', completed: true, date: '2023-05-14', description: 'Milk, eggs, vegetables' },
    { id: 4, title: 'Gym workout', category: 'Health', priority: 'Low', completed: false, date: '2023-05-15', description: 'Cardio and strength training' },
    { id: 5, title: 'Update portfolio website', category: 'Work', priority: 'High', completed: false, date: '2023-05-17', description: 'Add new projects and case studies' },
    { id: 6, title: 'Call mom', category: 'Personal', priority: 'Low', completed: true, date: '2023-05-14', description: 'Check on birthday plans' },
  ]);

  const [projects, setProjects] = useState([
    { id: 1, name: 'Website Redesign', progress: 75, tasks: 12, color: 'bg-indigo-500', deadline: '2023-06-15' },
    { id: 2, name: 'Mobile App', progress: 40, tasks: 8, color: 'bg-teal-500', deadline: '2023-07-10' },
    { id: 3, name: 'Marketing Campaign', progress: 30, tasks: 5, color: 'bg-amber-500', deadline: '2023-05-30' },
  ]);

  const [teamMembers, setTeamMembers] = useState([
    { id: 1, name: 'Alex Morgan', role: 'Designer', online: true, avatar: 'https://randomuser.me/api/portraits/women/44.jpg' },
    { id: 2, name: 'Jamie Smith', role: 'Developer', online: true, avatar: 'https://randomuser.me/api/portraits/men/32.jpg' },
    { id: 3, name: 'Taylor Johnson', role: 'Project Manager', online: false, avatar: 'https://randomuser.me/api/portraits/women/68.jpg' },
    { id: 4, name: 'Jordan Williams', role: 'QA Engineer', online: true, avatar: 'https://randomuser.me/api/portraits/men/22.jpg' },
  ]);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-gray-50">
          ...
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-6 text-white mb-6">
            ...
            <p className="opacity-90">You have {pendingTasks} pending tasks. {pendingTasks > 0 ? "Let's get things done!" : "Great job!"}</p>
            <div className="mt-4 flex items-center">
              ...
              <span className="ml-4 text-sm font-medium">{stats.productivity}% Productivity</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="border-b border-gray-200">
                  <div className="flex">
                    ...
                    <button 
                      className={`flex-1 py-4 px-6 text-center font-medium ${activeTab === 'health' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-medium text-gray-800">Productivity</h3>
                  <span className="text-sm font-medium text-indigo-600">{stats.weeklyGoal}% Weekly Goal</span>
                </div>
                ...
                <div className="space-y-4">
                  <div className="bg-gradient-to-r from-cyan-50 to-indigo-50 rounded-lg p-4">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-bold text-indigo-600">{stats.dailyCompleted}/15</span>
                    </div>
                    <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="bg-indigo-600 h-2 rounded-full" 
                        style={{ width: `${(stats.dailyCompleted / 15) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-indigo-50 p-4 rounded-lg">
                      ...
                      <p className="text-xl font-bold text-indigo-700 mt-1">{completedTasks}</p>
                      <p className="text-xs text-gray-500 mt-1">Tasks</p>
                    </div>
                    <div className="bg-amber-50 p-4 rounded-lg">
                      <p className="text-sm text-gray-600">In Progress</p>
                      <p className="text-xl font-bold text-amber-700 mt-1">{pendingTasks}</p>
                      <p className="text-xs text-gray-500 mt-1">Tasks</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <button 
                    ...
                    className={`flex-shrink-0 h-6 w-6 rounded-full border flex items-center justify-center ${showTaskDetails.completed ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300'}`}
                  >
                  ...
                  <h4 className={`ml-3 text-lg font-bold ${showTaskDetails.completed ? 'line-through text-gray-400' : 'text-gray-800'}`}>
                    {showTaskDetails.title}
                  </h4>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full ${showTaskDetails.category === 'Work' ? 'bg-indigo-100 text-indigo-800' : showTaskDetails.category === 'Health' ? 'bg-emerald-100 text-emerald-800' : 'bg-purple-100 text-purple-800'}`}>
